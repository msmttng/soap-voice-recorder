...
const GeminiClient = {
  ...
  _filterGenericDrugNames(drugInfo) {
    ...
    // 行が商品名（ブランド）識別子を持つか確認
    function hasMakerBracket(str) {
      return /「[^」]+」|（[^）]+）|\([^)]+\)/.test(str);
    }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (prevLine) {
        // 【安全ルール】現在行がメーカー識別子を持つ場合は絶対に除去しない
        if (!hasMakerBracket(line)) {
          const normLine = normalize(line);
          const normPrev = normalize(prevLine);

          // 判定1: 化学名語尾パターン（塩酸塩・フマル酸塩等）+ 前行より原文が長いか同等
          const isGenericSuffix = /塩酸塩|硫酸塩|硝酸塩|酒石酸塩|フマル酸塩|マレイン酸塩|クエン酸塩|リン酸塩|臭化水素酸塩|ナトリウム|カリウム|カルシウム|マグネシウム|水和物|無水物|エステル|安息香酸塩/.test(line);

          // 判定2: 正規化後に前行に含まれる（原文の長さで比較）
          ...
          if (isGenericSuffix || isSubstringOfPrev || endsWithDosageForm) {
            // 前行が商品名らしい場合のみ除去
            const prevIsBrandLike = hasMakerBracket(prevLine)
              || /[A-Za-zｦ-ﾟ]/.test(prevLine)
              || /mg|μg|ｍｇ|μｇ|AG|OD錠|DS/.test(prevLine);
            if (prevIsBrandLike) {
              const reason = isSubstringOfPrev ? '部分文字列' : endsWithDosageForm ? '配合剤形' : '化学名語尾';
